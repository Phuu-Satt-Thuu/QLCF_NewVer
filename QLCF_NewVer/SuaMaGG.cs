...
{
    public partial class SuaMaGG : Form
    {
        ...
        public SuaMaGG(int? maVC)
        {
            ...
            if (_voucherToEdit == null)
            {
                MessageBox.Show("Không tìm thấy mã giảm giá trong CSDL.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Load += (s, e) => this.Close();
            }
        }

        private void SuaMaGG_Load(object sender, EventArgs e)
        {
            LoadComboBoxes();
            LoadDataGridView();
            LoadDataToForm();
        ...
        {
            if (string.IsNullOrEmpty(txtTenMaGG.Text))
            {
                MessageBox.Show("Tên mã giảm giá không được để trống.", "Thiếu thông tin", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (maLoaiVC == 1) // Giảm %
                {
                    _voucherToEdit.GiaTri = numGiamPhanTram.Value;
                }
                else if (maLoaiVC == 3) // Giảm tiền
                {
                    _voucherToEdit.GiaTri = decimal.TryParse(txtGiaTriGiam.Text, out decimal gt) ? gt : 0;
                }

                // --- BƯỚC 3: CẬP NHẬT BẢNG CHITIETVC (CON) NẾU LÀ BOGO ---
                ...
                MessageBox.Show("Cập nhật mã giảm giá thành công!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
